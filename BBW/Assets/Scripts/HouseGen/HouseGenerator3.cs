...
public class HouseGenerator3 : MonoBehaviour
{
    ...
    public float navMeshAgentRadius = 0.4f;

    private Dictionary<int, NavMeshSurface> navMeshSurfaces = new Dictionary<int, NavMeshSurface>();
    ...
    private readonly Dictionary<int, Transform> floorRoots = new();
    private readonly Dictionary<int, NavMeshSurface> floorSurfaces = new();
    ...
    void BuildNavMeshes()
    {
        foreach (var kvp in occupiedBoundsPerFloor)
        {
            int floor = kvp.Key;
            List<Bounds> boundsList = kvp.Value;
            ...
            for (int i = 1; i < boundsList.Count; i++)
                combinedBounds.Encapsulate(boundsList[i]);

            GameObject navObj = new GameObject($"NavMesh_Floor_{floor}");
            navObj.transform.parent = transform;
            ...
            surface.agentTypeID = UnityEngine.AI.NavMesh.GetSettingsByIndex(0).agentTypeID;

            surface.center = combinedBounds.center;
            surface.size = combinedBounds.size + Vector3.up * 2f;

            navMeshSurfaces[floor] = surface;
        }

        foreach (var surface in navMeshSurfaces.Values)
        {
            surface.RemoveData();
    ...
    void EnsureFloorColliders()
    {
        foreach (var room in placedRooms)
        {
            GameObject go = room.gameObject; // ← IMPORTANT LINE

            if (!go) continue;

            // Skip if colliders already exist
            if (go.GetComponentInChildren<Collider>())
                continue;

            foreach (var r in go.GetComponentsInChildren<MeshRenderer>())
            {
                if (!r.gameObject.GetComponent<Collider>())
                {
        ...
        Debug.Log(
            $"[NavMesh] Colliders in scene now: " +
            Object.FindObjectsByType<Collider>(FindObjectsSortMode.None).Length
        );

    }

    Transform GetOrCreateFloorRoot(int floor)
    {
        if (floorRoots.TryGetValue(floor, out var t)) return t;

        var go = new GameObject($"FloorRoot_{floor}");
        go.transform.SetParent(transform, worldPositionStays: true);
        floorRoots[floor] = go.transform;
        return go.transform;
    }
    
    void ParentRoomsUnderFloorRoots()
    {
        foreach (var room in placedRooms)
        {
            if (!room.gameObject) continue;

            Transform root = GetOrCreateFloorRoot(room.floor);
            room.gameObject.transform.SetParent(root, worldPositionStays: true);
        }
    }
